...
export function generateVerificationEmailHtml(verificationUrl: string, locale: string = 'en') {
  ...
  return `
    ...
    <body>
      ...
      <div class="content">
        <p>${isZh ? '您好！' : 'Hello!'}</p>
        <p>${isZh ? '感谢您注册ImageFlux！请点击下面的按钮验证您的邮箱地址：' : 'Thank you for signing up for ImageFlux! Please click the button below to verify your email address:'}</p>
        ...
        <p>
          ${isZh ? '如果您没有注册ImageFlux账户，请忽略此邮件。' : 'If you did not sign up for ImageFlux, please ignore this email.'}
        </p>
      </div>
      <div class="footer">
        <p>&copy; 2025 ImageFlux. ${isZh ? '保留所有权利。' : 'All rights reserved.'}</p>
      </div>
    </body>
    </html>
  `;
...
export function generatePasswordResetEmailHtml(resetUrl: string, locale: string = 'zh'): string {
  ...
  return `
    ...
    <head>
      ...
      <title>${isZh ? 'ImageFlux - 密码重置' : 'ImageFlux - Password Reset'}</title>
      <style>
        body {
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>${isZh ? '此邮件由 ImageFlux 自动发送，请勿回复。' : 'This email was sent automatically by ImageFlux. Please do not reply.'}</p>
          <p>&copy; 2025 ImageFlux. ${isZh ? '保留所有权利。' : 'All rights reserved.'}</p>
        </div>
      </div>
    </body>
